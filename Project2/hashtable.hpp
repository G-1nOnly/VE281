...
protected:                                                                 // DO NOT USE private HERE!
    ...
    size_t findMinimumBucketSize_helper(const std::size_t *begin, const std::size_t *end, size_t size)
    {
        //Helper function of "findMinimumBucketSize"
        if (begin + 1 == end)
        ...
        if (*mid == size)
            ...
            return findMinimumBucketSize_helper(begin, mid + 1, size);
        else
            return findMinimumBucketSize_helper(mid + 1, end, size);
    }

    size_t findMinimumBucketSize(size_t bucketSize)
    {
        // TODO: implement this function
        size_t Size_Floor = (size_t)((double)tableSize / maxLoadFactor);
        bucketSize = (bucketSize > Size_Floor) ? bucketSize : Size_Floor;
        const std::size_t *begin = std::begin(HashPrime::g_a_sizes);
        const std::size_t *end = std::end(HashPrime::g_a_sizes);
        return findMinimumBucketSize_helper(begin, end, bucketSize);
    }
...
public:
    ...
    HashTable(const HashTable &that) : buckets(that.buckets), tableSize(that.tableSize),
    ...
    {
        // TODO: implement this function
        typename HashTableData::iterator it = buckets.begin() + hashKey(key);
        if (it->empty())
            return end();
        Iterator iter = Iterator(this, it, it->before_begin());
        while (iter.bucketIt == it)
        {
            if (keyEqual(iter->first, key))
                return iter;
            iter++;
        }
        return end();
    ...
    bool insert(const Iterator &it, const Key &key, const Value &value)
    {
        ...
        if (it == end())
        {
            ...
            typename HashTableData::iterator it = buckets.begin() + hashKey(key);
            it->push_front(temp);
            size_t comp = (size_t)(firstBucketIt - buckets.begin());
            if (comp > hashKey(key))
                firstBucketIt = it;
            rehash(buckets.size());
            return true;
